  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.pdfezy.com/api/pdf/pdf_to_pptx`,
        formData,
        {
      ...
      dispatch(setAction("pdf_to_pptx")); // Save action in global store
      setUploading(false);
      if (!auth) {
        ...
        if (
          ...
          try {
            await downloadFile(fileName, "pdf_to_pptx", token, router.push);
          } catch (err) {
  ...
  return (
    <main className="bg-gray-50">
      ...
      <section className="bg-blue-50 text-center py-16 px-4">
        <h1 className="text-4xl font-bold mb-4">PDF to PowerPoint Converter</h1>
        <p className="mb-6">
