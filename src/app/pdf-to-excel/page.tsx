  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.pdfezy.com/api/pdf/pdf_to_excel`,
        formData,
        {
      ...
      dispatch(setAction("pdf_to_excel")); // Save action in global store
      setUploading(false);
      if (!auth) {
        ...
        if (
          ...
          try {
            await downloadFile(fileName, "pdf_to_excel", token, router.push);
          } catch (err) {
