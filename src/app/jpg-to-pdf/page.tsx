  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.pdfezy.com/api/pdf/jpg_to_pdf`,
        formData,
        {
      ...
      dispatch(setAction("jpg_to_pdf")); // Save action in global store
      setUploading(false);
      if (!auth) {
        ...
        if (
          ...
          try {
            await downloadFile(fileName, "jpg_to_pdf", token, router);
          } catch (err) {
  ...
  return (
    <main className="bg-gray-50">
      ...
      <section className="bg-blue-50 text-center py-16 px-4">
        ...
        <p className="mb-6">
          We’ve processed 75,000 JPGs so far. Let’s make yours next.
        </p>
        <FileUploadSection
          acceptType=".jpg, .jpeg"
